...
export function VehicleForm({ brands, showrooms, vehicle, error }: VehicleFormProps) {
  ...
  return (
    <form action={action} className="admin-card vehicle-form">
      ...
      <section id="specs"><h2>Thông số kỹ thuật</h2><p className="admin-section-note">Các trường này được hiển thị thành bốn nhóm thông số trên trang chi tiết xe.</p><div className="admin-form-grid"><label>Động cơ<input name="engine" defaultValue={vehicle?.engine} /></label><label>Dung tích động cơ<input name="displacement" defaultValue={vehicle?.specifications.displacement} placeholder="Ví dụ: 2.998 cc" /></label><label>Công suất (mã lực)<input name="horsepower" type="number" min="0" defaultValue={vehicle?.horsepower ?? 0} /></label><label>Mô-men xoắn (Nm)<input name="torque" type="number" min="0" defaultValue={vehicle?.torque ?? 0} /></label><label>Tăng tốc 0–100 (giây)<input name="acceleration" type="number" min="0" step="0.1" defaultValue={vehicle?.acceleration ?? 0} /></label><label>Tốc độ tối đa<input name="maxSpeed" defaultValue={vehicle?.specifications.maxSpeed} placeholder="Ví dụ: 250 km/h" /></label><label>Mức tiêu thụ nhiên liệu<input name="consumption" defaultValue={vehicle?.specifications.consumption} placeholder="Ví dụ: 7,9–8,5 l/100km" /></label><label>Hộp số<input name="transmission" defaultValue={vehicle?.transmission} /></label><label>Dẫn động<input name="drivetrain" defaultValue={vehicle?.drivetrain} /></label><label>Kích thước Dài × Rộng × Cao<input name="dimensions" defaultValue={vehicle?.dimensions} /></label><label>Chiều dài cơ sở<input name="wheelbase" defaultValue={vehicle?.specifications.wheelbase} /></label><label>Khoảng sáng gầm xe<input name="groundClearance" defaultValue={vehicle?.specifications.groundClearance} /></label><label>Trọng lượng không tải<input name="curbWeight" defaultValue={vehicle?.specifications.curbWeight} /></label><label>Dung tích khoang hành lý<input name="luggageCapacity" defaultValue={vehicle?.specifications.luggageCapacity} /></label><label>Dung tích bình nhiên liệu / pin<input name="energyCapacity" defaultValue={vehicle?.specifications.energyCapacity} /></label></div><h3 className="admin-subheading">Tiện nghi</h3><div className="admin-form-grid"><label>Màn hình trung tâm<input name="centerScreen" defaultValue={vehicle?.specifications.centerScreen} /></label><label>Hệ thống âm thanh<input name="audio" defaultValue={vehicle?.specifications.audio} /></label><label>Điều hòa<input name="climate" defaultValue={vehicle?.specifications.climate} /></label><label>Ghế ngồi<input name="seatMaterial" defaultValue={vehicle?.specifications.seatMaterial} /></label><label>Cửa sổ trời<input name="sunroof" defaultValue={vehicle?.specifications.sunroof} /></label><label>Sạc không dây<input name="wirelessCharging" defaultValue={vehicle?.specifications.wirelessCharging} /></label><label>Kết nối<input name="connectivity" defaultValue={vehicle?.specifications.connectivity} /></label><label>Đèn viền nội thất<input name="ambientLighting" defaultValue={vehicle?.specifications.ambientLighting} /></label></div><h3 className="admin-subheading">An toàn</h3><div className="admin-form-grid"><label>Hệ thống hỗ trợ lái<input name="driverAssist" defaultValue={vehicle?.specifications.driverAssist} /></label><label>Kiểm soát hành trình<input name="cruiseControl" defaultValue={vehicle?.specifications.cruiseControl} /></label><label>Cảnh báo lệch làn<input name="laneWarning" defaultValue={vehicle?.specifications.laneWarning} /></label><label>Hỗ trợ giữ làn<input name="laneKeepAssist" defaultValue={vehicle?.specifications.laneKeepAssist} /></label><label>Cảnh báo điểm mù<input name="blindSpot" defaultValue={vehicle?.specifications.blindSpot} /></label><label>Phanh khẩn cấp<input name="emergencyBrake" defaultValue={vehicle?.specifications.emergencyBrake} /></label><label>Camera<input name="camera" defaultValue={vehicle?.specifications.camera} /></label><label>Cảm biến<input name="sensors" defaultValue={vehicle?.specifications.sensors} /></label><label>Túi khí<input name="airbags" defaultValue={vehicle?.specifications.airbags} /></label></div></section>
      <section id="content"><h2>Nội dung chi tiết</h2><p className="admin-section-note">Mỗi phần có thể dùng một ảnh riêng để bài giới thiệu xe phong phú hơn.</p><div className="admin-form-grid"><label>Tiêu đề tổng quan<input name="overviewTitle" defaultValue={vehicle?.overviewTitle} placeholder="Khẳng định vị thế tiên phong." /></label><label>Trích dẫn nổi bật<input name="overviewQuote" defaultValue={vehicle?.overviewQuote} placeholder="Một câu nhấn mạnh về mẫu xe" /></label></div><label>URL brochure PDF<input name="brochureUrl" type="url" defaultValue={vehicle?.brochureUrl} placeholder="https://.../brochure.pdf" /></label><div className="admin-form-grid content-editor-grid"><label>Ngoại thất<textarea name="exterior" rows={4} defaultValue={vehicle?.exterior} /><span>Ảnh phần Ngoại thất</span><input name="exteriorImage" type="url" defaultValue={vehicle?.exteriorImage} placeholder="https://..." /></label><label>Nội thất<textarea name="interior" rows={4} defaultValue={vehicle?.interior} /><span>Ảnh phần Nội thất</span><input name="interiorImage" type="url" defaultValue={vehicle?.interiorImage} placeholder="https://..." /></label><label>Công nghệ<textarea name="technology" rows={4} defaultValue={vehicle?.technology} /><span>Ảnh phần Công nghệ</span><input name="technologyImage" type="url" defaultValue={vehicle?.technologyImage} placeholder="https://..." /></label><label>An toàn<textarea name="safety" rows={4} defaultValue={vehicle?.safety} /><span>Ảnh phần An toàn</span><input name="safetyImage" type="url" defaultValue={vehicle?.safetyImage} placeholder="https://..." /></label></div></section>
