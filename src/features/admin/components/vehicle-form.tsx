...
export function VehicleForm({ brands, showrooms, vehicle, error }: VehicleFormProps) {
  ...
  return (
    <form action={action} className="admin-card vehicle-form">
      ...
      <section id="content"><h2>Nội dung chi tiết</h2><div className="admin-form-grid"><label>Ngoại thất<textarea name="exterior" rows={4} defaultValue={vehicle?.exterior} /></label><label>Nội thất<textarea name="interior" rows={4} defaultValue={vehicle?.interior} /></label><label>Công nghệ<textarea name="technology" rows={4} defaultValue={vehicle?.technology} /></label><label>An toàn<textarea name="safety" rows={4} defaultValue={vehicle?.safety} /></label></div></section>
      ...
      <section id="images"><h2>Hình ảnh</h2><label>URL ảnh đại diện *<input name="coverImage" type="url" required defaultValue={vehicle?.coverImage} placeholder="https://..." /></label><label>Các URL gallery<textarea name="galleryImages" rows={5} defaultValue={vehicle?.galleryImages} placeholder="Mỗi URL trên một dòng" /></label><small>Cloudinary có thể được kết nối sau; hiện form lưu URL ảnh an toàn vào database.</small></section>
